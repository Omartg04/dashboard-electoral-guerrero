...
with tab4:
    ...
    # Filtrar secciones muestrales
    scatter_data = filtered_gdf[filtered_gdf['SECCIÓN'].isin(df_sample['SECCIÓN'])].copy()
    
    # Depuración: Verificar secciones antes del merge
    st.write(f"Secciones en filtered_gdf (muestrales): {len(scatter_data)}")
    if scatter_data.empty:
        st.warning("No hay secciones muestrales disponibles con los filtros seleccionados.")
    else:
        ...
        scatter_data = scatter_data.merge(
            ...
            on='SECCIÓN',
            how='left',
            suffixes=('_gdf', '_sample')
        )
        
        # Depuración: Verificar columnas después del merge
        st.write("Columnas en scatter_data después del merge:", scatter_data.columns.tolist())
        ...
        if missing_columns:
            ...
            st.write("Primeras filas de scatter_data:", scatter_data.head().to_dict())
            st.write("Primeras filas de df_sample:", df_sample[['SECCIÓN', 'ENCUESTAS_ASIGNADAS', 'ENCUESTAS_ASIGNADAS_MUESTRAL', 'ENCUESTAS_REALIZADAS_MUESTRAL']].head().to_dict())
            st.stop()
        
        # Verificar que no haya valores nulos en columnas clave
        if scatter_data['ENCUESTAS_REALIZADAS_MUESTRAL'].isnull().all():
            st.warning("Advertencia: Todos los valores de 'ENCUESTAS_REALIZADAS_MUESTRAL' son nulos. Verifica los datos en df_sample.")
    ...
    if not scatter_data.empty:
        ...
        st.warning("No hay datos disponibles para el gráfico de intensidad con los filtros seleccionados.")
